...
import 'package:localserver_video_player/User/inherits_sub_folder.dart';
import 'package:localserver_video_player/User/preview.dart';
import 'package:localserver_video_player/User/video_player.dart';
...
class _Sub_FolderState extends State<Sub_Folder> {
  ...
  Future<List<Folder>> getData() async {
    ...
    setState(() {
      ...
      for (int a = 0; a < datavideo.length; a++) {
        if (datavideo[a].path.contains(widget.path)) {
          var substr_folder_initiative =
              datavideo[a].path.toString().lastIndexOf('/');
          var name_folder_initiative = (datavideo[a]
              .path
              .toString()
              .substring(0, substr_folder_initiative));
    ...
    for (int a = 0; a < filter_datavideo.length; a++) {
      ...
      String name_folder =
          (filter_datavideo[a].toString().substring(substr_folder + 1));
